...
- **프로그레스 바 및 카트 아이콘 애니메이션 개선**: 장보기 버튼 클릭 시 크롤링 로딩 속도를 초당 3%씩 천천히 차오르도록 늦추고, 80% 이후에는 99%까지 극도로 조금씩만 전진하도록 변경하여 실제 종료 전에 게이지가 미리 다 차버리는 인상을 주지 않도록 개선했습니다. 또한, 카트 아이콘이 버튼 테두리 밖으로 절반 잘려서 노출되던 문제를 보완하기 위해 JS `calc()` 동적 오프셋 식을 적용하여 `0%`부터 `100%`까지 카트 전체가 온전히 내부에서 노출되도록 해결했으며, 크롤링 완료 시 즉시 100%까지 빠르게 가속하여 게이지가 가득 차고 페이지가 갱신되도록 하였습니다.
